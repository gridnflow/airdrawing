...
const JUDGE_PROMPT = `이 이미지는 사용자가 웹캠 앞에서 손가락으로 허공에 그린 그림입니다. 선이 거칠고 손떨림이 있을 수 있습니다.
...
export async function recognizeDrawing(pngDataUrl: string): Promise<string> {
  const text = await callVision(pngDataUrl, PROMPT, false)
  return text || '(응답 없음)'
}

/** 게임 채점: 제시어를 알려주지 않고 한 단어 추측만 받아온다 (치팅 방지). */
export async function judgeDrawing(pngDataUrl: string): Promise<string> {
  const text = await callVision(pngDataUrl, JUDGE_PROMPT, true)
  try {
    const parsed = JSON.parse(text) as { guess?: string }
    return (parsed.guess ?? '').trim()
  } catch {
    return text.trim()
  }
}
